...
class ANR(nn.Module):
	...
	def __init__(self, logger, args, num_users, num_items):
		...
		##########################Modification#########################################
		#Multicriteria rating prediction
		self.multi_criteria=Criteria(self.num_users,self.num_items)

		# Overall rating prediction from criteria rating
		self.overall_criteria=Overall(10)
	...
	def forward(self, batch_uid, batch_iid, verbose = 0):
		...
		###################Modification#########################################
		pred_criteria_rating=self.multi_criteria(batch_uid, batch_iid)

		pred_criteria_rating=torch.concat([pred_criteria_rating, rating_pred], dim=1)
		pred_overall_rating=self.overall_criteria(pred_criteria_rating)

		#***********************************************************no need
		# rating_sum=rating_pred.add(pred_overall_rating)
		# overall_rating_pred=torch.div(rating_sum, 2.0)

		#print(f'\nPrint the share of overall_raing: {pred_overall_rating.shape}, and rating :{rating_pred.shape}')

		#return overall_rating_pred ##########rating_pred#, pred_overall_rating
		#overall_rating=sum(rating_pred,overall_criteria_rating)/2.0
		#######################MENDS############################################

		#return rating_pred
		return pred_overall_rating
