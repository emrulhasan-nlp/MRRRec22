...
#parser.add_argument("-d", "--dataset", type = str, default = "tripadvisor", help = "Dataset (Default: tripadvisor)")
parser.add_argument("-d", "--dataset", type = str, default = "clean_tripdata", help = "Dataset (Default: clean_tripdata)")
#parser.add_argument("-d", "--dataset", type = str, default = "BeerAdvSample", help = "Dataset (Default: BeerAdvSample)")
#parser.add_argument("-d", "--dataset", type = str, default = "RateBeerSample", help = "Dataset (Default: RateBeerSample)")
#parser.add_argument("-d", "--dataset", type = str, default = "InstantVideo", help = "Dataset (Default: InstantVideo)")
#parser.add_argument("-d", "--dataset", type = str, default = "toys", help = "Dataset (Default: beauty)")
...
REVIEW_JSON = "{}{}.csv".format( SOURCE_FOLDER, CATEGORY )
# ============================================================================= INPUT =============================================================================
...
##############################modification#####################################
user_item="../interactions/"
output_train	= "{}{}".format( user_item,fp_train)
output_test	= "{}{}".format( user_item, fp_test )
output_dev	= "{}{}".format( user_item, fp_dev )
...
interactions = []
import csv, json
with open(REVIEW_JSON, encoding="utf8") as csv_file:
	csv_reader=csv.DictReader(csv_file)
	for line in tqdm(csv_reader, "Initial pass of reviews for \"{}\"".format( CATEGORY )):
		item=line['HotelId']
		user=line['Author']

		interactions.append([user,item])

# #########modification ends heres##########
...
###################second modification starts here#####################
...
with open(REVIEW_JSON, encoding="utf8") as csv_file:
    ...
    for line in tqdm(csv_reader, "Initial pass of reviews for \"{}\"".format( CATEGORY )):
        ...
        if (hit( (user, item), users_dict, items_dict )):
            ...
            rating=float(rating)
            text=line['Content']
            #text = simple_tokenizer(line['t5_summary'])
            interactions.append([user, item, rating, text])
#text = simple_tokenizer(line['Content'])
			#break
...
# 			rating=float(rating)
# 			text = simple_tokenizer(line['review'])
...
#             rating=line['overall']
#             rating=float(rating)
#             text = simple_tokenizer(line['reviewText'])
#             interactions.append([user, item, rating, text])
...
interactions[:] = [interaction for interaction in tqdm(interactions, "Filtering interactions") 
	if (len(interaction[3]) >= MIN_REVIEW_LEN)]
...
if(args.dev_test_in_train):
	...
	print(f'dev interaction length: {len(dev_interactions)}')
	print(f'test interaction length: {len(test_interactions)}')
	
	newDevSize = len(dev_interactions)

	newTestSize = len(test_interactions)
	print(f'Newdev interaction length: {newDevSize}')
	print(f'Newtest interaction length: {newTestSize}')	
